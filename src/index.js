...
const BlockzieLink = require('blockzie-link');
const BlockzieResourceServer = require('blockzie-resource');
const ProgressBar = require('electron-progressbar');

const formatMessage = require('format-message');
const locales = require('blockzie-l10n/locales/link-desktop-msgs');
...
const createWindow = () => {
    mainWindow = new BrowserWindow({
        icon: path.join(__dirname, './icon/Blockzie-Link.ico'),
        width: 400,
        height: 400,
        center: true,
        resizable: false,
    ...
    // start link server
    const link = new BlockzieLink(dataPath, path.join(resourcePath, 'tools'));
    link.listen();

    // start resource server
    resourceServer = new BlockzieResourceServer(dataPath, path.join(resourcePath, 'external-resources'));
    resourceServer.listen();


    appTray = new Tray(nativeImage.createFromPath(path.join(__dirname, './icon/Blockzie-Link.ico')));
    appTray.setToolTip('Blockzie Link');
    appTray.setContextMenu(Menu.buildFromTemplate(makeTrayMenu(locale)));
